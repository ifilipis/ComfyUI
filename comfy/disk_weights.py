...
def module_to(module: torch.nn.Module, *args, **kwargs):
    if disk_weights_enabled():
        ...
        target_dtype = _extract_to_dtype(args, kwargs)
        if target_dtype is None:
            target_dtype = getattr(module, "manual_cast_dtype", None)
        if target_device is None:
            target_device = _find_existing_device(module) or torch.device("cpu")
        materialize_module_tree(module, target_device, dtype_override=target_dtype)
