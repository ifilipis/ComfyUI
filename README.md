...
| `--low-ram` | Enable disk weight offloading, set default RAM headroom to 1024MB, and use `--reserve-vram` as the only VRAM headroom. |
...
When `--low-ram` is set, ComfyUI streams safetensors weights from disk and keeps 1024MB of RAM headroom while using `--reserve-vram` for VRAM headroom.
