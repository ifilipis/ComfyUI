...
| `--low-ram` | Enable disk offloading for model weights with 3-tier memory management (VRAM → RAM → disk). |
...
When `--low-ram` is set, ComfyUI streams safetensors weights from disk and manages VRAM and RAM as a 3-tier cache (VRAM → RAM → disk), evicting least-recently-used weights when headroom is exceeded.
