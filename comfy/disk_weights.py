...
class DiskTensorRef:
    ...
    def load(
        ...
        if device is not None and tensor.device != device:
            ...
            tensor = model_management.cast_to(tensor, device=device, non_blocking=non_blocking)
        return tensor
...
class DiskWeightCache:
    ...
    def _current_tensor(self, module: torch.nn.Module, entry: CacheEntry) -> Optional[torch.Tensor]:
        if entry.is_buffer:
            return module._buffers.get(entry.name)
        return module._parameters.get(entry.name)

    def _is_entry_stale(self, entry: CacheEntry) -> bool:
        module = entry.module_ref()
        if module is None:
            return True
        tensor = entry.tensor_ref()
        if tensor is None:
            return True
        current = self._current_tensor(module, entry)
        if current is None:
            return True
        return current is not tensor

    def _drop_entry(self, key: tuple[int, str], entry: CacheEntry):
    ...
    def pop_lru_entry(self) -> Optional[CacheEntry]:
        while self._entries:
            key, entry = self._entries.popitem(last=False)
            if self._is_entry_stale(entry):
                self._drop_entry(key, entry)
...
RAM_CACHE = DiskWeightCache(0)
VRAM_CACHE = DiskWeightCache(None)
...
def configure(cache_bytes: int, allow_gds: bool, pin_if_cpu: bool, enabled: bool = True):
    ...
    RAM_CACHE.set_limit(cache_bytes if enabled else 0)
    VRAM_CACHE.set_limit(None if enabled else 0)
    if not enabled:
        RAM_CACHE._entries.clear()
        RAM_CACHE.total_bytes = 0
        VRAM_CACHE._entries.clear()
        VRAM_CACHE.total_bytes = 0
...
def _drop_cache_entry(module: torch.nn.Module, name: str):
    RAM_CACHE.remove_entry(module, name)
    VRAM_CACHE.remove_entry(module, name)


def _drop_module_cache(module: torch.nn.Module):
    RAM_CACHE.remove_module(module)
    VRAM_CACHE.remove_module(module)


def _record_cache(module: torch.nn.Module, name: str, tensor: torch.Tensor, is_buffer: bool):
    _drop_cache_entry(module, name)
    if tensor.device.type == "cpu":
        RAM_CACHE.record(module, name, tensor, is_buffer=is_buffer)
    elif tensor.device.type == "cuda":
        VRAM_CACHE.record(module, name, tensor, is_buffer=is_buffer)


def _touch_cache(module: torch.nn.Module, name: str, tensor: torch.Tensor):
    if tensor.device.type == "cpu":
        RAM_CACHE.touch(module, name)
    elif tensor.device.type == "cuda":
        VRAM_CACHE.touch(module, name)
        ...
        for name, param in submodule.named_parameters(recurse=False):
            ...
            if key in state_dict:
                ...
                if param.device.type != "meta":
                    _record_cache(submodule, name, param, is_buffer=False)
        for name, buf in submodule.named_buffers(recurse=False):
            ...
            if key in state_dict and buf is not None:
                ...
                if buf.device.type != "meta":
                    _record_cache(submodule, name, buf, is_buffer=True)
...
def _evict_ram_for_budget(required_bytes: int) -> int:
    if required_bytes <= 0:
        return 0
    return evict_ram_bytes(required_bytes)
...
class _BudgetedStateDict(MutableMapping):
    ...
    def get_tensor(
        ...
        *,
        ...
        if key in self._overrides:
            t = self._overrides[key]
            if (device is not None and t.device != device) or (requested_dtype is not None and t.dtype != requested_dtype):
                from . import model_management
                non_blocking = model_management.device_supports_non_blocking(device) if device is not None else False
                t = model_management.cast_to(t, dtype=requested_dtype, device=device, non_blocking=non_blocking)
            return t
...
def ensure_module_materialized(
    ...
    for name in sorted(refs.keys()):
        ...
        if current.device.type != "meta" and current.device == target_device and (
            ...
            _touch_cache(module, name, current)
            continue
        ...
        if target_device.type == "cpu":
            ...
            if free_mem < required_bytes:
                evict_ram_bytes(required_bytes - free_mem)
        else:
            free_mem = _device_free_memory(target_device)
            if free_mem < required_bytes:
                evict_vram_bytes(required_bytes - free_mem)
        ...
        if current.device.type == "meta":
            ...
            offload_stream = None
            if target_for_load is not None and current.device != target_for_load:
                offload_stream = model_management.get_offload_stream(target_for_load)
        ...
        if tensor.device.type != "meta":
            _record_cache(module, name, tensor, is_buffer=is_buffer)
    _rebuild_materialization_state(module, refs, state)
...
def evict_ram_bytes(bytes_to_free: int):
    if bytes_to_free <= 0:
        return 0
    return RAM_CACHE.evict_bytes(bytes_to_free)


def evict_ram_cache(bytes_to_free: int):
    return evict_ram_bytes(bytes_to_free)


def evict_vram_bytes(bytes_to_free: int):
    if bytes_to_free <= 0:
        return 0
    from . import model_management
    freed = 0
    while freed < bytes_to_free:
        entry = VRAM_CACHE.pop_lru_entry()
        if entry is None:
            break
        module = entry.module_ref()
        if module is None:
            continue
        current = entry.tensor_ref()
        if current is None:
            continue
        if current.device.type != "cuda":
            continue
        required_bytes = entry.size_bytes
        free_mem = model_management.get_free_memory(torch.device("cpu"))
        if free_mem < required_bytes:
            evict_ram_bytes(required_bytes - free_mem)
            free_mem = model_management.get_free_memory(torch.device("cpu"))
        if free_mem >= required_bytes:
            non_blocking = model_management.device_supports_non_blocking(torch.device("cpu"))
            demoted = model_management.cast_to(current, device=torch.device("cpu"), non_blocking=non_blocking)
            if entry.is_buffer:
                module._buffers[entry.name] = demoted
            else:
                requires_grad = False
                refs = REGISTRY.get(module)
                if refs and entry.name in refs:
                    requires_grad = refs[entry.name].requires_grad
                module._parameters[entry.name] = torch.nn.Parameter(demoted, requires_grad=requires_grad)
            _record_cache(module, entry.name, demoted, is_buffer=entry.is_buffer)
            freed += required_bytes
            continue
        _evict_module_weight(module, entry.name, entry.is_buffer)
        freed += required_bytes
    return freed
...
def move_module_tensors(module: torch.nn.Module, device_to: torch.device, dtype_override: Optional[torch.dtype] = None):
    def _move(tensor):
        ...
        offload_stream = model_management.get_offload_stream(device_to) if tensor.device != device_to else None
    ...
    if refs:
        for name, ref in refs.items():
            ...
            if tensor is None or tensor.device.type == "meta":
                _drop_cache_entry(module, name)
                continue
            _record_cache(module, name, tensor, is_buffer=ref.is_buffer)
    return module
    ...
    *,
    ...
    if current.device.type != "meta":
        if current.device != device or (target_dtype is not None and current.dtype != target_dtype):
            from . import model_management
            non_blocking = model_management.device_supports_non_blocking(device)
            offload_stream = model_management.get_offload_stream(device) if current.device != device else None
            tensor = model_management.cast_to(
                current,
                dtype=target_dtype,
                device=device,
...
def _materialize_module_from_state_dict(
    ...
    for name, param in module.named_parameters(recurse=False):
        if param.device.type != "meta":
            _record_cache(module, name, param, is_buffer=False)
    for name, buf in module.named_buffers(recurse=False):
        if buf is not None and buf.device.type != "meta":
            _record_cache(module, name, buf, is_buffer=True)
