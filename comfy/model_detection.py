...
def sd_shape(state_dict, key):
    return comfy.utils.state_dict_tensor_shape(state_dict, key)
...
def detect_unet_config(state_dict, key_prefix, metadata=None):
    ...
    if '{}cap_embedder.1.weight'.format(key_prefix) in state_dict_keys:  # Lumina 2
        ...
        if dit_config["dim"] == 2304: # Original Lumina 2
            ...
            if ctd_key in state_dict:  # NewBie
...
def convert_diffusers_mmdit(state_dict, output_prefix=""):
    if comfy.safetensors_stream.is_stream_state_dict(state_dict):
        return _convert_diffusers_mmdit_streaming(state_dict, output_prefix=output_prefix)
    out_sd = {}
...
def _convert_diffusers_mmdit_streaming(state_dict, output_prefix=""):
    if 'joint_transformer_blocks.0.attn.add_k_proj.weight' in state_dict: #AuraFlow
        num_joint = count_blocks(state_dict, 'joint_transformer_blocks.{}.')
        num_single = count_blocks(state_dict, 'single_transformer_blocks.{}.')
        sd_map = comfy.utils.auraflow_to_diffusers({"n_double_layers": num_joint, "n_layers": num_joint + num_single}, output_prefix=output_prefix)
    ...
    for k in sd_map:
        ...
        drop_keys.add(k)
        if isinstance(t, str):
            contributions.setdefault(t, []).append((k, None, None))
        else:
            fun = t[2] if len(t) > 2 else (lambda a: a)
            contributions.setdefault(t[0], []).append((k, t[1], fun))

    for out_key, entries in contributions.items():
        first_key, first_offset, _ = entries[0]
        meta = state_dict.meta(first_key)
        out_shape = list(meta.shape)
        for in_key, offset, _ in entries:
            in_meta = state_dict.meta(in_key)
            if offset is not None:
                dim = offset[0]
                size = offset[1] + offset[2]
                out_shape[dim] = max(out_shape[dim], size)
            else:
                out_shape = list(in_meta.shape)
        out_numel = 1
        for dim in out_shape:
            out_numel *= dim
        out_meta = comfy.safetensors_stream.TensorMeta(
            dtype=meta.dtype,
            shape=tuple(out_shape),
            numel=out_numel,
            nbytes=out_numel * torch.tensor([], dtype=meta.dtype).element_size(),
            data_offsets=None,
            filename=meta.filename,
        )

        def _loader(entries=entries, out_shape=tuple(out_shape)):
            output = None
            for in_key, offset, fun in entries:
                weight = state_dict[in_key]
                weight = fun(weight)
                if offset is None and len(entries) == 1:
                    return weight
                if output is None:
                    output = torch.empty(out_shape, device=weight.device, dtype=weight.dtype)
                if offset is None:
                    output.copy_(weight)
                else:
                    view = output.narrow(offset[0], offset[1], offset[2])
                    view.copy_(weight)
            return output

        extra[out_key] = comfy.safetensors_stream.DerivedEntry(_loader, out_meta)

    return comfy.safetensors_stream.CompositeStateDict(state_dict, extra=extra, drop_keys=drop_keys)
