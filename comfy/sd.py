...
class CLIP:
    ...
    def load_sd(self, sd, full_model=False):
        ...
        model_management.register_disk_refs(self.cond_stage_model, sd)
        return out
    ...
    def __init__(self, sd=None, device=None, config=None, dtype=None, metadata=None):
        ...
        if config is None:
            if "decoder.mid.block_1.mix_factor" in sd:
                ...
                self.latent_channels = comfy.utils.state_dict_tensor_shape(sd, "taesd_decoder.1.weight")[1]
                ...
                conv_shape = comfy.utils.state_dict_tensor_shape(sd, "decoder.conv_in.weight")
                if conv_shape[1] == 64:
                    ddconfig = {"block_out_channels": [128, 256, 512, 512, 1024, 1024], "in_channels": 3, "out_channels": 3, "num_res_blocks": 2, "ffactor_spatial": 32, "downsample_match_channel": True, "upsample_match_channel": True}
                    self.latent_channels = ddconfig['z_channels'] = conv_shape[1]
                    self.downscale_ratio = 32
                ...
                elif conv_shape[1] == 32 and len(conv_shape) == 5:
                    ...
                    if 'post_quant_conv.weight' in sd:
                        self.first_stage_model = AutoencoderKL(ddconfig=ddconfig, embed_dim=comfy.utils.state_dict_tensor_shape(sd, "post_quant_conv.weight")[1])
                    else:
                        self.first_stage_model = AutoencodingEngine(regularizer_config={'target': "comfy.ldm.models.autoencoder.DiagonalGaussianRegularizer"},
                ...
                tensor_conv1_shape = comfy.utils.state_dict_tensor_shape(sd, "decoder.up_blocks.0.res_blocks.0.conv1.conv.weight")
                version = 0
                ...
                self.working_dtypes = [torch.bfloat16, torch.float32]
            elif "decoder.conv_in.conv.weight" in sd and comfy.utils.state_dict_tensor_shape(sd, "decoder.conv_in.conv.weight")[1] == 32:
                ddconfig = {"block_out_channels": [128, 256, 512, 1024, 1024], "in_channels": 3, "out_channels": 3, "num_res_blocks": 2, "ffactor_spatial": 16, "ffactor_temporal": 4, "downsample_match_channel": True, "upsample_match_channel": True}
                ddconfig['z_channels'] = comfy.utils.state_dict_tensor_shape(sd, "decoder.conv_in.conv.weight")[1]
                ...
                self.latent_dim = 3
                self.latent_channels = ddconfig['z_channels'] = comfy.utils.state_dict_tensor_shape(sd, "decoder.conv_in.conv.weight")[1]
                self.first_stage_model = AutoencoderKL(ddconfig=ddconfig, embed_dim=comfy.utils.state_dict_tensor_shape(sd, "post_quant_conv.weight")[1])
                ...
                if "decoder.upsamples.0.upsamples.0.residual.2.weight" in sd:  # Wan 2.2 VAE
                    ...
                    dim = comfy.utils.state_dict_tensor_shape(sd, "decoder.head.0.gamma")[0]
                    self.upscale_ratio = (lambda a: max(0, a * 4 - 3), 8, 8)
                    self.upscale_index_formula = (4, 8, 8)
                    self.downscale_ratio = (lambda a: max(0, math.floor((a + 3) / 4)), 8, 8)
                    self.downscale_index_formula = (4, 8, 8)
                    self.latent_dim = 3
                    self.latent_channels = 16
                    self.output_channels = comfy.utils.state_dict_tensor_shape(sd, "encoder.conv1.weight")[1]
            ...
            # Hunyuan 3d v2 2.0 & 2.1
                ...
                self.latent_channels = comfy.utils.state_dict_tensor_shape(sd, "decoder.1.weight")[1]
                self.latent_dim = 3
                self.upscale_ratio = (lambda a: max(0, a * 4 - 3), 16, 16)
                self.upscale_index_formula = (4, 16, 16)
                self.downscale_ratio = (lambda a: max(0, math.floor((a + 3) / 4)), 16, 16)
                ...
                if self.latent_channels == 48: # Wan 2.2
                ...
                elif self.latent_channels == 32 and comfy.utils.state_dict_tensor_shape(sd, "decoder.22.bias")[0] == 12: # lighttae_hv15
                    ...
                    if comfy.utils.state_dict_tensor_dtype(sd, "decoder.1.weight") == torch.float16: # taehv currently only available in float16, so assume it's not lighttaew2_1 as otherwise state dicts are identical
        ...
        model_management.register_disk_refs(self.first_stage_model, sd)
...
def load_diffusion_model_state_dict(sd, model_options={}, metadata=None):
    ...
    if model_config is not None:
        ...
        if new_sd is not None: #diffusers mmdit
            ...
            if comfy.safetensors_stream.is_stream_state_dict(sd):
                mapping = {}
                for k in diffusers_keys:
                    if k in sd:
                        mapping[diffusers_keys[k]] = k
                    else:
                        logging.warning("{} {}".format(diffusers_keys[k], k))
                new_sd = comfy.safetensors_stream.RenameViewStateDict(sd, mapping)
                if hasattr(sd, "discard_keys"):
                    sd.discard_keys(mapping.values())
            else:
                new_sd = {}
                for k in diffusers_keys:
                    if k in sd:
