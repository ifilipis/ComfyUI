...
| `--low-ram` | Enable disk-backed weight offloading/streaming for model weights. |
...
When `--low-ram` is set, ComfyUI streams safetensors weights from disk and evicts weights back to disk under memory pressure. RAM headroom is enforced for CPU allocations; use `--cache-ram` to override the default headroom.
